...
# Autoresearch for Copilot CLI — Autonomous Goal-directed Iteration
...
## Copilot CLI Mode Integration

This skill integrates with Copilot CLI's native **plan mode** and **autopilot mode** at two key transition points:

| Phase | Trigger | Tool | Recommended Action |
|-------|---------|------|--------------------|
| `/autoresearch:plan` — Phase 7 (Confirm & Launch) | Config validated, baseline confirmed | `exit_plan_mode` | `autopilot` |
| `/autoresearch` — loop launch (after setup + baseline) | All preconditions met, baseline established | `exit_plan_mode` | `autopilot` |
...
**How it works:**
- Use `exit_plan_mode` with `summary` = the formatted config block, `recommendedAction: autopilot`, and `actions: [autopilot, interactive, exit_only]`
...
See `references/plan-workflow.md` (Phase 7) and `references/autonomous-loop-protocol.md` (Copilot CLI Launch Gate) for the exact instructions.
