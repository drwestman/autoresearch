...
## Copilot CLI Launch Gate

**After Phase 0 passes and the baseline metric is established — before entering Phase 1 — use `exit_plan_mode` to present the validated configuration and transition to the appropriate execution mode.**

Build the summary block from the confirmed config:

```markdown
## Autoresearch Ready to Launch ✓
...
**Mode:** {unlimited | bounded — N iterations}
```
...
- `autopilot` → Enter Phase 1 immediately. The loop runs uninterrupted in autopilot mode — never pause to ask "should I continue?".
- `interactive` → Enter Phase 1. The loop runs normally; the user may interrupt between iterations.
- `exit_only` → Print the full configuration and baseline, then stop. Do not enter the loop.
...
- `git revert` is also safer in Copilot CLI — it's a non-destructive operation that doesn't trigger safety warnings.
