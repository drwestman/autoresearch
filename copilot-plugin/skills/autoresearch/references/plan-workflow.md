...
Build the complete configuration summary:
...
Use the `exit_plan_mode` tool to present the config and let the user choose how to proceed:
...
**After `exit_plan_mode` returns, act on the selected action:**
...
- `exit_only` → Output the ready-to-paste command block and stop. Do not launch the loop.
