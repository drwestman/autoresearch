...
metadata:
  hermes:
    tags: [autonomous, iteration, optimization, debugging, security-audit, shipping, scenarios, prediction, reasoning]
    related_skills: []
---
...
2. **If ANY required context is missing → you MUST use `clarify` to collect it BEFORE proceeding to any execution phase.** DO NOT skip this step. DO NOT proceed without user input.
...
| "autoresearch plan" | Goal | Ask via `clarify` per `references/plan-workflow.md` |
...
This skill integrates with Hermes Agent's native **plan mode** and **autopilot mode** at two key transition points:
...
See `references/plan-workflow.md` (Phase 7) and `references/autonomous-loop-protocol.md` (Hermes Agent Launch Gate) for the exact instructions.
...
**CRITICAL: If ANY critical field is missing (Goal, Scope, Metric, Direction, or Verify), you MUST use `clarify` to collect them interactively. DO NOT proceed to The Loop or any execution phase without completing this setup. This is a BLOCKING prerequisite.**

### Interactive Setup (when invoked without full config)

Scan the codebase first for smart defaults, then ask ALL questions in batched `clarify` calls (max 4 per call). This gives users full clarity upfront.

**Batch 1 — Core config (4 questions in one call):**

Use a SINGLE `clarify` call with these 4 questions:
...
**IMPORTANT:** You MUST call `clarify` with batched questions — never ask one at a time, and never skip this step. Users should see all config choices together for full context. DO NOT proceed to Setup Steps or The Loop without completing interactive setup.
