...
**CRITICAL — BLOCKING PREREQUISITE:** If invoked without `--mode` or sufficient inline context, you MUST use `clarify` to gather config BEFORE proceeding to Phase 1.

**TOOL AVAILABILITY:** `clarify` may be a deferred tool. If calling it fails, use tool discovery to fetch the schema first, then retry. NEVER skip setup because of tool issues.
...
### Questions (single clarify call — 4 questions)
...
4. **Strategy by count:**
   - **0 files:** Warn via clarify: "No docs found. Switch to init mode? [yes/cancel]". If yes → restart as init. If cancel → STOP.
