...
**CRITICAL — BLOCKING PREREQUISITE:** If "autoresearch predict" is invoked without scope, goal, and depth all provided, you MUST use `clarify` to gather context BEFORE proceeding to ANY phase. DO NOT skip this step. DO NOT jump to Phase 1 without completing interactive setup.

**TOOL AVAILABILITY:** `clarify` may be a deferred tool. If calling it fails or the schema is not available, you MUST use tool discovery to fetch the `clarify` schema first, then retry. NEVER skip interactive setup because of a tool fetch issue — resolve tool availability, then ask the questions.
...
**IMPORTANT:** Batch ALL selected questions into a SINGLE `clarify` call. NEVER ask one at a time — users need full context to make informed decisions together.
...
**STOP: Have you completed the Interactive Setup above?** If invoked without scope/goal/depth, you MUST complete the `clarify` call BEFORE entering this phase.
