...
**CRITICAL — BLOCKING PREREQUISITE:** If "autoresearch reason" is invoked without task, domain, and mode all provided, you MUST use `clarify` to gather context BEFORE proceeding to Phase 1. DO NOT skip this step.

**TOOL AVAILABILITY:** `clarify` may be a deferred tool. If calling it fails, use tool discovery to fetch the schema first, then retry.
...
Batch ALL selected questions into a SINGLE `clarify` call:
...
**STOP: Have you completed the Interactive Setup above?** Complete `clarify` before entering this phase.
