...
**CRITICAL — BLOCKING PREREQUISITE:** If "autoresearch security" is invoked without `--diff`, scope, or focus, you MUST scan the codebase first, then use `clarify` to gather user input BEFORE proceeding to ANY phase. DO NOT skip this step.

**Single batched call — all 3 questions at once:**

You MUST call `clarify` with all 3 questions in ONE call:
...
jobs:
  security-audit:
    ...
    steps:
      ...
      - name: Install GitHub Hermes extension
