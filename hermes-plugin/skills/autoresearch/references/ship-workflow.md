...
**CRITICAL — BLOCKING PREREQUISITE:** If "autoresearch ship" is invoked without `--type` or target, you MUST scan for staged changes, open PRs, and recent commits, then use `clarify` to gather user input BEFORE proceeding to ANY phase. DO NOT skip this step.
